...
def floyd(adj_mat: np.ndarray, layers, seq_num_nodes, node_types, query_type, enc_dist):
    if query_type not in TYPE_TO_DIST_MAT:
        ...
        # 8, 9
        if enc_dist == 'e' :
            next = 8 + seq_num_nodes - 2
            dist[0, 2:seq_num_nodes] = dist[2:seq_num_nodes, 0] = np.arange(8, next)  # 9,10, 11

            next += (seq_num_nodes - 2)
            dist[1, 2:seq_num_nodes] = dist[2:seq_num_nodes, 1] = np.arange(next, next + seq_num_nodes - 2)
        else :
...
def encode_filled_query(query, query_type, variables, tail, enc_dist='d'):
    if query_type in ['2u-DNF', 'up-DNF']:
        if query_type == '2u-DNF':
            queries = [query[0], query[1]]
            query_type = '1p'
        ...
        __handle_neg_query(query)
        query = list2tuple(query)
        return __encode_filled_query(query, query_type, tail, enc_dist)
    else:
        return __encode_filled_query(query, query_type, tail, enc_dist)

def __encode_filled_query(query, query_type, enc_dist):
    ...
    if 'n' in query_type:
        tgt_type = 1
        neg_h, neg_r = query_type_to_neg_ind[query_type]
        negs[neg_h] = 2
        negs[neg_r] = 1
    # ==================================================================================================================
...
if __name__ == '__main__':
    train_queries = pickle.load(open(os.path.join('data/FB15k-237-betae', "train-queries.pkl"), 'rb'))
    # qtype = 'pin'
    # query_struct = query_type_to_structure[qtype]
    # train_queries = list(train_queries[query_struct])
    for query_structure in train_queries :
        qtype = query_structure_to_type[query_structure]
        for q in list(train_queries[query_structure]) :
            print(qtype)
            print(q)
            data = encode_query(q, qtype, 'd')
            x = data['x']
            qtype_idx = data['query_type']
            negs = data['negs']
            print(x)
            print(negs)
            print()
        ...
        # if (160, (140,)) in q and (740, (74,)) in q:
        #     print(q)
        #     print(x)
