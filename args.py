...
def parse_args(args=None):
    ...
    parser.add_argument('--enc_dist', choices=['u', 'd', 'n', 'no', 'e'], default='d',
                        help='u: undirected dist, d: directed dist, n: only neighbors, e: extended dist')
    parser.add_argument('--ignore_1p', action='store_true')
    # ================================================================================================
    parser.add_argument('--note', default='mae', type=str)
    parser.add_argument('--alpha_l', default=3, type=int)
    parser.add_argument('--mask_rate', default=0.4, type=float)
    # ================================================================================================
    parser.add_argument('--cqd_ckpt_path', default='cqd_e1000_d100', type=str)
    parser.add_argument('--mp_hidden_dim', default=4096, type=int)
    parser.add_argument('--mp_layers', default=1, type=int)
    parser.add_argument('--eps', default=0.1, type=float)
    parser.add_argument('--agg_func', default='sum')
    parser.add_argument('--recon', default='all')
    parser.add_argument('--pretrained', action='store_true', default=False)
    parser.add_argument('--pred', default='tgt_pred', type=str, choices=['tgt_pred', 'tgt_pred_ns', 'tgt_pred_ce', 'ghgr_pred', 'qtype_pred', 'no', 'tgt_pred_lmpnn'])
    parser.add_argument('--enc_mask', default='mask', type=str)
    parser.add_argument('--dec_mask', default='mask', type=str)
    parser.add_argument('--not_input_masking', action='store_true', default=False)

    parser.add_argument('--spc_tok_mask', default=True, type=bool)
    parser.add_argument('--mask_cands', default='node', type=str)
    parser.add_argument('--mp_rel', default=False, type=bool)
    # parser.add_argument('--mp_aug', default=False, type=bool)

    parser.add_argument('--mp_lhs', default=True, type=bool)

    parser.add_argument('--swap_enc_task', default='no', type=str)
...
def post_init_args(args):
    ...
    # ========================================================================================================================
    if len(tasks) == 14 :
        args.save_path = os.path.join(prefix, args.data_path.split('/')[-1], args.geo, args.note)
    else :
        args.save_path = os.path.join(prefix, args.data_path.split('/')[-1], args.geo, ori_task + '_' + args.note)
    # ========================================================================================================================
    ...
    #     shutil.copy('models/modeling_bert_dist.py', os.path.join(args.save_path, 'modeling_bert_dist.py'))
