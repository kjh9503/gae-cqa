...
IDX_TO_TYPE = {pair[1] : pair[0] for pair in TYPE_TO_IDX.items()}
...
def log_metrics_qtype(mode, step, metrics, qtype=''):
    '''
    Print the evaluation logs
    '''
    for metric in metrics:
        logging.info('%s %s %s at step %d: %f' % (qtype, mode, metric, step, metrics[metric]))
