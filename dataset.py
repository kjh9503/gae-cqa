...
from query_format_converter import convert_query_to_pyg, get_inv_rel, convert_query_to_postfix_notation, \
    convert_query_to_adjacent_matrix, encode_query, encode_filled_query, TGT, VAR, OFFSET
...
qtype_to_mask = {'1p' : [[3, 4], [9, 9], [9, 9], [9, 9], [9, 9]],
                 '2p' : [[3, 4], [3, 4], [4, 5], [5, 6], [9, 9]],
                 '3p' : [[3, 4], [4, 5], [5, 6], [6, 7], [7, 8]],
                 '2i' : [[4, 6], [5, 6], [9, 9], [9, 9], [9, 9]],
                 '3i' : [[5, 8], [6, 8], [7, 8], [9, 9], [9, 9]],
                 '2in' : [[4, 6], [5, 6], [9, 9], [9, 9], [9, 9]],
                 '3in' : [[5, 8], [6, 8], [7, 8], [9, 9], [9, 9]],
                 'inp' : [[4, 6], [5, 6], [6, 7], [7, 8], [9, 9]],
                 'pin' : [[3, 4], [4, 6], [6, 8], [7, 8], [9, 9]],
                 'pni' : [[3, 4], [4, 6], [6, 8], [7, 8], [9, 9]]
                 }
...
def flatten_query_filled(queries):
    all_queries = []
    for query_structure in queries:
        tmp_queries = list(queries[query_structure])
        all_queries.extend([(query, query_structure_to_type[query_structure]) for query in tmp_queries])
...
def flatten_query_(query_structure, queries):
    all_queries = []
    tmp_queries = list(queries)
    all_queries.extend([(query, query_structure_to_type[query_structure]) for query in tmp_queries])
    return all_queries

def load_data(args, tasks, train=True, valid=True, test=True):
    '''
...
class Q2TTrainDataset_Filled(TrainDataset):
    def __init__(self, queries, num_ent, num_rel, negative_sample_size, answer, data_path,
                 enc_dist='d', filled_queries=None):
        super().__init__(queries, num_ent, num_rel, negative_sample_size, answer, data_path)

        self.query_to_graph = None
        self.filled_queries = filled_queries
        self.enc_dist = enc_dist
        save_path = Path(data_path) / 'encoded_train_queries.pkl'
        if not DEV:
            self.query_to_graph = {}
    ...
    def __getitem__(self, idx):
        ...
        # ============================================================================
        tgt_ind = qtype_to_spc_token_ind[query_type][TGT]
        filled_x = copy.deepcopy(batch['x'])
        filled_x[tgt_ind] = tail + OFFSET
        if self.filled_queries is not None:

            var_inds = qtype_to_spc_token_ind[query_type][VAR]
            if query_type == '3p' :
                variables_ind = random.choice(list(range(len(self.filled_queries[query][tail]))))
                variables = list(self.filled_queries[query][tail])[variables_ind]
                filled_x[var_inds[0]] = variables[0] + OFFSET
                filled_x[var_inds[1]] = variables[1] + OFFSET
            else :
                variables = np.random.choice(list(self.filled_queries[query][tail]))
                filled_x[var_inds[0]] = variables + OFFSET
        batch['filled_x'] = filled_x
        # ============================================================================
        for k in batch:
            batch[k] = torch.LongTensor(batch[k])
...
class Q2TTrainDataset_Branch(TrainDataset):
    def __init__(self, queries, num_ent, num_rel, negative_sample_size, answer, data_path,
                 enc_dist='d', branch_mask_rate=3):
        super().__init__(queries, num_ent, num_rel, negative_sample_size, answer, data_path)

        self.query_to_graph = None
        self.enc_dist = enc_dist
        self.branch_mask_rate = branch_mask_rate
        save_path = Path(data_path) / 'encoded_train_queries.pkl'
    ...
    def __getitem__(self, idx):
        ...
        query_type = self.queries[idx][1]
        # ============================================================================================================
        mask_ind_cands = torch.LongTensor(qtype_to_mask[query_type])
        mask_inds = torch.LongTensor(np.random.choice(len(mask_ind_cands), self.branch_mask_rate, replace=False))
        branch_mask = mask_ind_cands[mask_inds]
        branch_mask = torch.LongTensor(branch_mask).flatten()
        # ============================================================================================================
        ...
        batch.update({'branch_mask': branch_mask})
        return batch
